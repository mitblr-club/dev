  ...
  return (
    <div className="mt-c25 flex h-c60 flex-col items-center justify-between overflow-x-hidden md:mt-c15 md:h-c30 lg:h-c30 xl:mt-c5 xl:h-c45">
      ...
      <div className="flex justify-between gap-c3 md:gap-c10">
        <div>
          <Link href="/events">
            <button className="text-decoration-none transition-box-shadow user-select-none -webkit-user-select-none touch-action-manipulation will-change-box-shadow active:shadow-inner-lg relative box-border inline-flex h-12 cursor-pointer items-center justify-center overflow-hidden whitespace-nowrap rounded-md border-none bg-primary px-4 text-left text-base font-bold text-white shadow-md transition-transform will-change-transform hover:-translate-y-2 hover:shadow-lg hover:ring-2 hover:secondary hover:ring-opacity-50 focus:outline-none focus:ring-2 focus:secondary focus:ring-opacity-50 active:translate-y-2 active:ring-2 active:secondary active:ring-opacity-50">
              Current Events
            </button>
          </Link>
        </div>
